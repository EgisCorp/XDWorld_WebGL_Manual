...
3DS 모델 파일을 사용하여 모델링 오브젝트 [JSGhostSymbol](../object/jsghostsymbol.md)를 추가하는 방법에 대해 소개합니다.

각 모델 데이터는 [JSGhostSymbolMap](../object/jsghostsymbolmap.md) 오브젝트에 저장되고, 저장된 모델 데이터는 [JSGhostSymbol](../object/jsghostsymbol.md) 오브젝트에서 형상 정보로 활용됩니다.
...
Module의 getGhostSymbolMap호출하여 JSGhostSymbolMap API 객체를 반환 받은 후 insert(링크) API로 모델 데이터를 등록합니다.
...
파라미터 속성 중 faceCount, faceTexture 정보는 다음 단계 모델 텍스쳐 설정(링크) 에서 활용됩니다.
...
오브젝트가 바닥면 중앙을 중심으로 지면에 서 있을 수 있도록 모델의 높이의 절반만큼 원점 위치를 낮춥니다.
...
레이어 타입은 Module.ELT_GHOST_3DSYMBOL 타입으로 설정합니다.
